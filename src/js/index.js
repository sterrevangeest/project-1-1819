...
export class API {
  ...
  _parsePartial(partial) {
    ...
    if (!regex.test(partial))
      throw new Error(`'${partial}' is not a valid endpoint and/or query.`);

    const { endpoint, value, max = 20, pagesize = 20 } = partial.match(
      regex
    ).groups;
    if (pagesize > 20)
      console.warn(
        `API supports, at most, 20 results at a time – not ${pagesize}.`
      );
    ...
    const query = (() => {
      switch (endpoint) {
        ...
        case "availability":
          return "id";
        default:
          throw new Error(`Unknown/unsupported endpoint '${endpoint}'`);
      }
    })();
    ...
    return {
      ...
      max: Number(max),
      pagesize: Math.min(Number(max), Number(pagesize))
    };
  }

  async _getRequestSpecifics(partial, options) {
    if (partial === undefined)
      throw new Error("Failed to get request specifics. Did you forget a url?");

    const { endpoint, query, max, pagesize } = Object.assign(
      {},
  ...
  async createStream(partial, options = {}) {
    const { batches, builtURL, count } = await this._getRequestSpecifics(
      partial,
      options
    );
  ...
  async createIterator(partial, options = {}) {
    ...
    async function* iterator() {
      const requests = range(batches).map(
        index => builtURL + `&page=${index + 1}&rctx=${this._context}`
      );
